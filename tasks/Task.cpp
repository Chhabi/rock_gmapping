...
::rock_gmapping::OccupancyGrid Task::getDynamicMap()
{
    boost::mutex::scoped_lock(map_mutex_);
    if(got_map_ && map_.width && map_.height)
        return map_;
    else
       return ::rock_gmapping::OccupancyGrid();
}
...
void Task::scanTransformerCallback(const base::Time &ts, const ::base::samples::LaserScan &scan)
{
  ...
  /* Since ROCK LaserScan supports only int ranges, using a new custom message type 'Laser' (see rock_gmappingTypes.hpp) to emulate ROS senson_msgs::LaserScan */
  Laser scan_sample;
  ...
  temp.setTransform(map_to_odom_.getTransform().inverse() * odom_to_laser.getTransform());
  _map_to_odom.write(temp);
  */
